...
function MapHandler({ bounds, mapRef, onMapClick }) {
    ...
    useEffect(() => {
        if (!bounds) return;
        map.setMaxBounds([[bounds.south, bounds.west], [bounds.north, bounds.east]]);
        const campusCenter = [(bounds.north + bounds.south) / 2, (bounds.east + bounds.west) / 2];
...
export default function CampusMapContainer({ searchQuery, selectedCategory, onBuildingSelect, isDarkMode }) {
    ...
    const pathFinder = useMemo(() => path.aStar(graph, {
    ...
    const handleBuildingClick = (building) => {
        ...
        if (startNode && shortestPath) {
            const pathCoords = shortestPath.map(n => [n.data.lat, n.data.lng]).reverse();
